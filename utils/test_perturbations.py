...
class TinyDomainIncScenario(TransformationIncremental):
    ...
    def get_task_trsf(self, task_index):
        return transforms.Compose(self.trsf.transforms + [self.inc_trsf[task_index]])
...
# dataset = ImageFolderDataset('data/tiny-32/train')

# class_ids = np.array(dataset.get_data()[1] < 5)
# data = [x[class_ids] for x in dataset.get_data() if x is not None]
# data.append(None)
# # dataset.dataset.targets = np.array(dataset.dataset.targets)[class_ids]
# # dataset.dataset.imgs = np.array(dataset.dataset.imgs)[class_ids]
# dataset = CustomDataset(data, 'data/tiny-32/train')
# scenario = Perturbations(dataset, list_perturbation=list_test, list_severity=[1])
